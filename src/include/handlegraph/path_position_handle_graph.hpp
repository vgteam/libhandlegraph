...
public:
    ...
    bool for_each_step_and_position_on_handle(const handle_t& handle, const Iteratee& iteratee) const;
...
protected:
    ...
    /// on a handle in any path. If it returns bool and returns false, stop iteration.
    /// Returns true if we finished and false if we stopped early.
    virtual bool for_each_step_and_position_on_handle(
...
template<typename Iteratee>
bool PathHandleGraph::for_each_step_and_position_on_handle(const handle_t& handle, const Iteratee& iteratee) const {
    return for_each_step_and_position_on_handle_impl(handle, BoolReturningWrapper<Iteratee, step_handle_t>::wrap(iteratee));
}
