...
void for_each_handle_in_shortest_path(const HandleGraph* g, handle_t start, handle_t end, std::function<bool(const handle_t&, size_t)> iteratee) {
    ...
    unordered_map<handle_t, pair<handle_t, distance>> traceback_map;
    ...
    while (!queue.empty()) {
        ...
        if (current == end) {
            ...
            //clear the queue just in case break doesn't work
            queue.clear();
            break;
        }
    ...
    vector<pair<handle_t, distance>> backwards_traceback;
    current = end;
    while (current != start) {
    ...
    //Go backwards through backwards_traceback and call iteratee on everything
    //backwards_traceback won't include start so do that with distance 0 too
    if (backwards_traceback.size() == 0) {
        iteratee(start, 0);
        return;
    }
    for (int i = backwards_traceback.size()-1 ; i >= 0 ; i--) {
